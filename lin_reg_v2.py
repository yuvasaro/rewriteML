# 10/25/2023
# Better linear regression with vectorization, feature scaling, and feature engineering!
...
def plot_J_hist(J_hist, num_iter):
    ...
    ax.set_ylabel("Cost")
    ax.set_xlabel(f"Iteration (in {math.ceil(num_iter / len(J_hist))}s)")
    plt.show()


def plot_prediction(x, X_norm, y_train, w_final, b_final):
    """Plots the model's prediction curve and the target values.

    Args:
        x (np.ndarray): x-axis values.
        X_norm (np.ndarray): Normalized matrix of engineered features.
        y_train (np.ndarray): Target values.
        w_final (np.ndarray): Final coefficient parameters.
        b_final (float): Final intercept parameter.
    """
    plt.scatter(x, y_train, marker='x', c='r', label="Actual Value"); plt.title("Trained Model Prediction")
    plt.plot(x, X_norm@w_final + b_final, label="Predicted Value"); plt.xlabel("x"); plt.ylabel("y"); plt.legend(); plt.show()
